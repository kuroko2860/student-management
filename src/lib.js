...
/**
 * Find the schedule that was effective for the requested month.
 *
 * scheduleHistory:
 * [
 *   {
 ...
 * ]
 */
export function scheduleForMonth(scheduleHistory = [], month) {
  const targetMonth = `${month}-01`;

  let result = null;

  for (const item of scheduleHistory) {
    if (item.effective_month <= targetMonth) {
      if (!result || item.effective_month > result.effective_month) {
        result = item;
      }
    }
  }

  return result?.schedule_slots || {};
}
...
/**
 * Sinh danh sách buổi học của một lớp trong tháng.
 *
 * The schedule is selected from schedule history based on the
 * requested month, so changing today's schedule never changes
 * already-passed months.
 */
export function buildSessions(month, classId, scheduleHistory = [], sheet) {
  const schedule = scheduleForMonth(scheduleHistory, month);
...
export function summarize(students, sessions, sheet, fee) {
  ...
  const rows = students.map((st) => {
    const off = (absences[st.id] || []).filter((id) =>
...
export function buildMessage(
  template,
  { row, cls, month, sessions, sheet, payment },
) {
  ...
  const ck = payment?.accountNo
    ? `Chuyển khoản: ${payment.accountNo} - ${
        payment.accountName || ""
      } (${payment.bankName || ""}), nội dung ghi tên con.`
    : "";
